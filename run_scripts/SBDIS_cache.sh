...
sub_blocks="2"         # sub_block choices: 2, 4, 8 
...
if [ "$fcache_type" = "IL1" ]; then
	...
	sed -i "/^\s*il1_cache.*/a \ \ \ \ sub_blocks\ =\ $sub_blocks" $configs_inter
	sed -i '/^\s*il1_cache.*/a \ \ \ \ faulty_cache\ =\ True' $configs_inter
	cp $configs_inter $configs_write
elif [ "$fcache_type" = "DL1" ]; then
    cp $configs_init $configs_inter
	sed -i "/^\s*dl1_cache.*/a \ \ \ \ sub_blocks\ =\ $sub_blocks" $configs_inter
...
cache_faulty_map=$fltmaps_dir"/"$fault_map_select"/"$pfail_select"/create-subitmap_sbdis"$sub_blocks".c"
...
do
  ...
  out_config="sbdis_cache_pfail_"$pfail_select"_subblk_"$sub_blocks
