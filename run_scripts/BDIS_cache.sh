...
if [ "$fcache_type" = "IL1" ]; then
  ...
  cp $configs_init $configs_write
  echo 'ERROR: wrong fcache_type: '$fcache_type' @ faulty'
  exit 1;
fi

cache_faulty_map=$fltmaps_dir"/"$fault_map_select"/"$pfail_select"/create-subitmap_bdis.c"
...
do
  ...
  out_config="BDIS_cache_pfail_"$pfail_select
